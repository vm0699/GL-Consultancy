...
export default function FeesClient() {
  ...
  // ✅ Popup should appear EVERY new browser session (not permanent)
  const [showLoginPopup, setShowLoginPopup] = useState(true);
  const [userInfo, setUserInfo] = useState<{ name: string; phone: string } | null>(null);
  const [isCheckingAuth, setIsCheckingAuth] = useState(true);

  // ✅ Use sessionStorage instead of localStorage (session-only)
  const STORAGE_KEY = "fee_page_user_session";

  // Check sessionStorage on mount - only CLOSE popup if user already submitted in THIS session
  useEffect(() => {
  ...
  const handleLoginSubmit = async (name: string, phone: string): Promise<void> => {
    // Call API to submit data to Google Sheets (server-side proxy)
  ...
  // Mandatory popup: if user tries to close, send them back to home
  ...
  const programs = useMemo(() => {
    const relevant = feeRecords.filter(
      (f) =>
  ...
  return (
    <main className="min-h-screen bg-[#faf8f3] text-[#1e2749] pb-20">
          ...
          <section className="mt-12">
            ...
            <div className="grid gap-4 sm:grid-cols-1 lg:grid-cols-2">
              {filteredFees.length === 0 ? (
                ...
                filteredFees.map((fee) => <FeeCard key={fee.id} record={fee} />)
              )}
            </div>
          </section>
      ...
      {/* ✅ Show popup only after auth check so it doesn't flash */}
      {!isCheckingAuth && (
        <LoginPopup
  ...
  return (
    <article
      className={`group rounded-3xl border ${
        isVit ? "border-amber-100 bg-white" : "border-transparent bg-white"
      } p-6 shadow-sm transition-all hover:shadow-md hover:border-amber-200`}
    >
          ...
          <div className="flex items-start justify-between">
            <span
              className={`px-2 py-1 rounded-lg ${
                isVit ? "bg-amber-100 text-amber-800" : "bg-gray-100 text-gray-700"
